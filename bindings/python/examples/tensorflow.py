#!/usr/bin/env python3
"""Runnable example: load safetensors with TensorFlow using TensorStore and run LLM inference.

Usage:
    python examples/tensorflow.py gpt2 --prompt "Hello, world!"
...
def run_inference(
    ...
    if fmt == "serverlessllm":
        ...
        if backend == "sync":
            from tensor_store_py.tensorflow import load_safetensors_sync
            ...
            state_dict = load_safetensors_io_uring(
                local_path, framework="tensorflow", device=device
            )
        else:
            from tensor_store_py.tensorflow import load_safetensors
    ...
    inputs = tokenizer(prompt, return_tensors="tf").to(device)
...
def main():
    parser = argparse.ArgumentParser(
        description="Load safetensors with TensorFlow using TensorStore and run inference"
    )
    parser.add_argument(
        "model",
    ...
    parser.add_argument(
        "--device",
        default="/CPU:0",
        help="Device to load tensors to (default: /CPU:0)",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
