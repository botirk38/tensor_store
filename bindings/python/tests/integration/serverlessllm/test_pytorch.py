...
def test_embedding_lookup_mmap(serverlessllm_dir, hidden_dim):
...
@pytest.mark.asyncio
async def test_embedding_lookup_async(serverlessllm_dir, hidden_dim):
...
def _attention_computation(weights, layer_idx=0, hidden_dim=126):
...
def test_attention_computation_mmap(serverlessllm_dir, hidden_dim):
...
@pytest.mark.asyncio
async def test_attention_computation_async(serverlessllm_dir, hidden_dim):
...
def _full_layer_forward(weights, layer_idx=0, hidden_dim=126):
...
def test_full_layer_forward_mmap(serverlessllm_dir, hidden_dim):
    """Test full layer forward with mmap-loaded weights."""
    output = _full_layer_forward(
        load_serverlessllm_mmap(serverlessllm_dir), hidden_dim=hidden_dim
    )
    assert output.shape == (2, 5, hidden_dim)
    assert not torch.isnan(output).any()
    assert not torch.isinf(output).any()
...
@pytest.mark.asyncio
async def test_full_layer_forward_async(serverlessllm_dir, hidden_dim):
    """Test full layer forward with async-loaded weights."""
    output = _full_layer_forward(
        await load_serverlessllm(serverlessllm_dir), hidden_dim=hidden_dim
    )
    assert output.shape == (2, 5, hidden_dim)
    assert not torch.isnan(output).any()
    assert not torch.isinf(output).any()
...
def _gradient_test(weights, layer_idx=0, hidden_dim=126):
...
def test_gradient_flow_sync(serverlessllm_dir, hidden_dim):
...
def test_gradient_flow_mmap(serverlessllm_dir, hidden_dim):
...
@pytest.mark.asyncio
async def test_gradient_flow_async(serverlessllm_dir, hidden_dim):
