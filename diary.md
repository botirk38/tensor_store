...
Repository hygiene and documentation sweep:
- Removed bundled `code.zip` from `submission/`; build zip archives from the tree when uploading instead of committing a second copy
- Stopped tracking `report/` and root `diary.md` in git (both listed in `.gitignore`) so local PDFs, LaTeX output, and the diary stay out of shared history
- Added missing module READMEs, module doc comments on Rust `src/bin/` implementations, and remaining `missing_docs` coverage on structs and impls

## 2026-04-15

Submission packaging and report polish:
- Regenerated `report/final_report.zip` and `paper.zip` with excludes for `.venv`, Cargo `target/`, benchmark `.cache` and `.benchmarks`, `.pytest_cache`, and LaTeX aux files
- Deleted one-off `report/final_report/_count_wc.py` and stray `.aux`, `.log`, `.out`, `.toc`, `.bbl`, `.blg` under `paper/` and `report/final_report/`
- Set declaration word count from `detex` over chapters `00_abstract` through `11_conclusion` (14,927 words at last check)
- Reviewed `paper/` and `report/final_report/` for assistant-style boilerplate; nothing beyond ordinary technical phrasing (including “large language model” where the topic is LLM inference)
