...
The final results show that backend performance is strongly workload-dependent. No backend is universally best. Instead, the winning backend depends on checkpoint layout, model size, and evaluation layer.
...
These results support the main thesis of this project: the practical goal is not to identify one universally superior backend, but to build a workload-aware \texttt{default} policy that stays close to the best explicit choice.
...
The key result is not that one backend dominates, but that the partitioned layout changes the ranking. This is exactly the kind of workload sensitivity that motivates an adaptive \texttt{default} policy.
...
Taken together, the results support an adaptive, format-aware default policy rather than any universal backend choice.
