...
The final conclusion of the project is simple but stronger than the original hypothesis. There is no universally best checkpoint-loading backend. The best backend depends on workload structure, checkpoint layout, and integration layer. What matters in practice is therefore not proving that one backend always wins, but designing a system that can select the right backend for the workload at hand.
