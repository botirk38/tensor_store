...
async fn profile_async(serverlessllm_dir: &str) {
    ...
    println!("  Warming up...");
    for i in 0..3 {
        let _warmup = serverlessllm::Model::load(serverlessllm_dir).await.unwrap();
    ...
    let model = serverlessllm::Model::load(black_box(serverlessllm_dir))
        .await
        .unwrap();
...
fn profile_sync(serverlessllm_dir: &str) {
    ...
    println!("  Warming up...");
    for i in 0..3 {
        let _warmup = serverlessllm::Model::load_sync(serverlessllm_dir).unwrap();
        println!("    Warmup iteration {} complete", i + 1);
    }
    ...
    println!("  Starting profiling...");
    let start = Instant::now();
    let model = serverlessllm::Model::load_sync(black_box(serverlessllm_dir)).unwrap();
    let load_time = start.elapsed();
