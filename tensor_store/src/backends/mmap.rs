...
fn ensure_range(file_len: u64, offset: u64, len: usize) -> IoResult<()> {
    let end = offset
        .checked_add(u64::try_from(len).map_err(|_e| {
            Error::new(ErrorKind::InvalidInput, "length too large for u64")
        })?)
    ...
    if end > file_len {
        return Err(Error::new(
            ...
            format!(
                "range {offset}..{end} exceeds file length {file_len} bytes",
            ),
        ));
    }
    Ok(())
...
pub fn load_blocking(path: impl AsRef<Path>) -> IoResult<Vec<u8>> {
    ...
    let file_len = usize::try_from(metadata.len()).map_err(|_e| {
        Error::new(ErrorKind::InvalidInput, "file too large")
    })?;
...
pub fn load_range_blocking(path: impl AsRef<Path>, offset: u64, len: usize) -> IoResult<Vec<u8>> {
    ...
    let aligned_offset = offset.checked_div(page_size).unwrap_or(0).checked_mul(page_size).unwrap_or(0);
    ...
    let end = start.checked_add(len).ok_or_else(|| Error::new(ErrorKind::InvalidInput, "range end overflow"))?;
    let slice = mmap.get(start..end).ok_or_else(|| Error::new(ErrorKind::InvalidInput, "invalid slice range"))?;
    Ok(slice.to_vec())
}
