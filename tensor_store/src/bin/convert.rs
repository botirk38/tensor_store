...
fn main() {
    ...
    if args.len() < 3 || args.len() > 4 {
        eprintln!("Usage: {} <input.safetensors> <output_dir> [partition_count]", args[0]);
    ...
    tokio_uring::start(async {
        match tensor_store::convert_safetensors_to_serverlessllm(input_path, output_dir, partition_count).await {
            Ok(()) => {
                println!("✓ Conversion completed successfully!");
                println!("  Index: {}/tensor_index.json", output_dir);
                println!("  Data files: {}/tensor.data_0 ... {}/tensor.data_{}",
                    output_dir, output_dir, partition_count - 1);
            }
            Err(e) => {
